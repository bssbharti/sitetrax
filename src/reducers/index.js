...
const hoistCamDirectorImages = (state: Array<HoistCamDirectorImage> = [], action) => {
  switch (action.type) {
    ...
    case types.SAVE_UPLOADED_IMAGES:
      console.log('inside the SAVE_UPLOADED_IMAGES reducer');
      return state.slice(0).concat(action.images);
    default:
      return state;
  }
};
...
const loggedIn = (state = false, action) => {
  switch (action.type) {
    case types.RECEIVE_LOGIN_STATUS:
